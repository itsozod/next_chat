...
const Sidebar = () => {
  ...
  return (
    <aside className="no-scrollbar sticky top-0 bottom-0 overflow-auto h-[100vh] bg-[#003049] w-20 lg:w-[400px] border-r border-base-300 flex flex-col transition-all duration-200">
      {isLoading ? (
          ...
          <div className="flex flex-col gap-3 items-start overflow-y-autopy-3">
            {rooms?.data?.map((room: Room) => (
              ...
              >
                ...
                <div
                  className={`${isMediumDevice ? "block w-full" : "flex"} text-white whitespace-nowrap overflow-hidden text-ellipsis sm:text-[1rem]`}
                >
                  {room.name}
                </div>
              </button>
